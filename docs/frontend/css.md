...
3. `flex-flow`: column wrap, combine `flex-direction` and `flex-wrap`
...
[Flexbox Froggy level 24 思路](https://flexboxfroggy.com/)

看到红色在最下边，说明`flex-direction`至少是 `column`，试一下，然后发现红色在上方，改成`column-reverse`

...
1. [下载对应的包](https://github.com/MetaCubeX/mihomo)，Debian 机器用的是 `mihomo-linux-amd64-v1.19.3.deb`
2. 安装包：`sudo apt install ./mihomo.deb`，不能少了 `./`，要不然不知道是本地包。默认安装到 `/usr/bin/mihomo`，配置在 `/etc/mihomo`，不知道在哪就用 `whereis` 查一下
3. 能命令行运行 `mihomo` 再继续

#### 配置

1. 把配置文件放到 `/etc/mihomo/config.yaml`，本地 `clash for windows 0.20.39` 配置即可，点击首页的 cpu 图标就是最后提交的配置。
2. 下载 [GeoLite2-Country.mmdb](https://github.com/Loyalsoldier/geoip)，启动 mihomo 时会下载，但是往往下载失败。手动下载后命名为 `Country.mmdb` 放在 `/etc/mihomo/` 文件夹下
3. 创建 service 来自动化，`sudo vim /etc/systemd/system/mihomo.service`
...
5. **必要的一步** ：如果想在 Linux 中让​命令行工具（如 curl、wget、git、apt 等）走代理，通常需要在 ~/.bashrc（或 ~/.zshrc）中设置 http_proxy 和 https_proxy 环境变量。**其实也能让 git 自己走代理**。这里让所有命令都走代理。
...
定时配置：我这个就没必要了，所以没配（`0 4 * * * curl -L -o /etc/mihomo/config.yaml -A "clash" SUB_URL; systemctl reload mihomo` 不明白可以问 AI）
